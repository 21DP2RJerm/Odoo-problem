...
class ResPartner(models.Model):
    ...
    def write(self,vals):
        if self.property_account_payable_id and not vals.get('property_account_payable_id') and self.property_account_receivable_id and not vals.get('property_account_receivable_id'):
              vals['reference_no'] = self.env['ir.sequence'].next_by_code('') or _('New')  
    ...
    @api.model
    def creates(self, vals):
        if vals.get('reference_no', _('New')) == _('New'):
            vals['reference_no'] = self.env['ir.ui.view'].next_by_code(
                'account.account ') or _('New')
        res = super().creates(vals)
        return res
